...
namespace MyTy.Blog.Web.Services
{
	public class PostScanner
	{
		...
		readonly IFileUpdater postUpdater;
		
		public PostScanner(BlogDB db)
		{
			this.db = db;
			this.postUpdater = new PostUpdater(db);
			this.rxDirectory = new ReactiveDirectory(@"~/Posts", "md");
		}

		public async Task Start()
		{
			subscriptions[0] = rxDirectory.UpdatedFiles.Subscribe(this.postUpdater.FileUpdated);
			subscriptions[1] = rxDirectory.DeletedFiles.Subscribe(this.postUpdater.FileDeleted);
			...
			var findDeletedFilesTask = Task.Factory.StartNew(() => { 
				...
				foreach (var file in deleteFiles) {
					this.postUpdater.FileDeleted(file);
				}
			});
